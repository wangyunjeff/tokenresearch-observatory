...
`final_answer` 由服务端从最终明确结论中提取，推荐标准化为字符串；前端只接受数字 `21` 或去除首尾空格后的字符串 `"21"`。不要用全文包含“21”的正则代替结论提取。不明确、否定式答案、多答案歧义应为 `null`，记为未通过。

有效回答 = 通过 + 未通过；失败、检测中、无数据不进入回答通过率分母。一个时段多条结果按“未通过 → 请求失败 → 检测中 → 通过”优先显示，弹窗保留每条记录。缺失/未来时间不填造成有数据。
