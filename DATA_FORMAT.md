...
- `pelicans`: 真实生成产物数组，页面最多展示前 200 条；在线服务端按最新到最旧排序，不混入参考素材或占位产物。
...
服务器事故恢复后可以追加真实补测记录，但不能重写原记录。补测记录保留实际请求 `timestamp`，并包含 `source: "recovery_retest"`、`display_timestamp`（被补的历史 10 分钟格）、`recovery_reason: "server_incident"` 和 `replaces`（原始故障记录 ID）。当补测完成时，页面将其归入 `display_timestamp` 对应的格子，并从模型统计中排除被替代的原始服务故障；原记录仍可在详情中查看。
...
有效回答 = 通过 + 未通过；失败、检测中、无数据不进入回答通过率分母。右上角“总体不降智率”使用累计糖果有效回答计算，即通过 ÷（通过 + 未通过），并展示对应通过次数与有效回答次数。一个时段多条结果若同时包含通过和未通过，页面显示“混合结果”并保留每条原始记录；其他组合仍按“未通过 → 请求失败 → 检测中 → 通过”显示。缺失/未来时间不填造成有数据。
