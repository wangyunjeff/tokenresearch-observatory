...
const stateFile=path.join(root,'runtime','integration-state.json');
await fs.rm(path.dirname(stateFile),{recursive:true,force:true});
const dataDir=path.join(root,'runtime','integration-archive');
...
const child=spawn(process.execPath,['server/main.mjs'],{cwd:root,env:{...process.env,HOST:'127.0.0.1',PORT:String(appPort),STATE_FILE:'runtime/integration-state.json',RUN_ON_START:'true',OPENAI_BASE_URL:`http://127.0.0.1:${apiPort}/v1`,OPENAI_API_KEY:'test-secret',CANDY_MODEL:'mock-model',PUBLIC_MODEL_LABEL:'ChatGPT',PELICAN_EXEC_MODE:'http',CODEX_EXEC_URL:`http://127.0.0.1:${execPort}/run`,CODEX_EXEC_KEY:'test-exec-key',REFERENCE_ARCHIVE_DIR:'runtime/integration-archive'},stdio:['ignore','pipe','pipe']});
...
}finally{child.kill('SIGTERM');api.close();exec.close();await fs.rm(path.dirname(stateFile),{recursive:true,force:true});}
